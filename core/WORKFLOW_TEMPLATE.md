# WORKFLOW.md — 사람-에이전트 반복 실행 루프

> Core Layer 문서. 모든 프로젝트 공통 작업 흐름 표준.
> Claude Code 세션 기반 환경을 전제.

---

## 1. 세 가지 루프

| 레벨 | 루프 | 주기 |
|------|-----|------|
| **Level 1** | 세션 루프 | 1회 Claude Code 세션 |
| **Level 2** | 기능 루프 | 1개 기능 완성 |
| **Level 3** | Phase 루프 | 1개 Phase 완료 |

---

## 2. 세션 시작 루틴 (Level 1)

에이전트는 세션 시작 시 순서대로 수행한다.

1. `CONSTITUTION.md` 읽기
2. `CLAUDE.md` 읽기
3. `docs/exec-plans/NEXT_SESSION.md` 확인 (이전 세션 핸드오프)
4. `PLANS.md` 현재 위치 + `open-decisions.md` 미결 항목 확인
5. 사람에게 한 줄 확인:
   > "오늘 작업: [추정 작업]. 맞으면 진행, 아니면 방향 알려주세요."

**예외**: 사람이 명확한 지시로 시작한 경우(예: "X 기능 구현해줘") → 5 생략.

---

## 3. 기능 크기 분류

기능 루프 시작 전, 에이전트는 작업 크기를 먼저 판단한다.

### 작은 기능 (Small)
아래 **3가지 모두** 해당:
- 데이터 스키마 변경 없음 (DB, 저장 포맷, API 스펙, JSON 스키마 등)
- 외부 API 신규 연동 없음
- CONSTITUTION 불변 원칙과 관련 없음

→ **에이전트 자체 완료 가능.**

### 큰 기능 (Large)
위 중 **하나라도** 해당 안 됨 **또는** 판단 불명확.

→ **사람 최종 승인 필요.** (불확실하면 큰 기능으로 간주)

> ⚠️ 이 기준은 운영하면서 조정 가능. 너무 자동화되면 엄격하게, 너무 번거로우면 느슨하게.

---

## 4. 기능 루프 (Level 2)

### 4-1. 작은 기능 흐름
```
[사람] 방향 결정
  → [Orchestrator] 크기 판단: Small + 직접 구현
  → [Reviewer subagent] 검증 (실패 시 3회 재시도)
  → [Orchestrator] 문서 동기화 (/sync-docs) + 커밋
  → [Orchestrator] 사후 보고
```

### 4-2. 큰 기능 흐름
```
[사람] 방향 결정
  → [Orchestrator] 크기 판단: Large + /orchestrate 실행
  → [Implementer subagent] 구현
  → [Reviewer subagent] 검증 (실패 시 3회 재시도 → 실패 시 에스컬레이션)
  → [Orchestrator] Reviewer PASS 보고 + 사람 승인 대기
  → [사람] 최종 승인
  → [Documenter subagent] 문서 업데이트 + 커밋
```

상세 흐름: `.claude/commands/orchestrate.md` §3-B

### 4-3. 작업 중 크기가 바뀐 경우
작은 기능으로 시작했는데 중간에 스키마 변경, 외부 API 필요 등이 발생하면
**즉시 멈추고 에스컬레이션**.

---

## 5. 사람의 개입 시점 (요약)

| 상황 | 사람 개입 필요? |
|------|--------------|
| 세션 시작 | ✅ |
| 작은 기능 전체 | ❌ |
| 큰 기능 시작 전 불명확함 | ✅ |
| 큰 기능 검증 후 최종 승인 | ✅ |
| 에스컬레이션 발생 | ✅ |
| CONSTITUTION 개정 | ✅ (사람만 가능) |
| AGENTS.md 자율 범위 내 결정 | ❌ |
| typecheck/lint 에러 자체 수정 | ❌ |
| tech-debt 자동 등록 | ❌ |
| 세션 종료 시 | ✅ |

---

## 6. 에스컬레이션 조건

에이전트는 아래 상황에서 **즉시 멈추고** 사람에게 보고한다.
상세: `ESCALATION_PROTOCOL.md`

- 두 선택지가 동등하게 합리적일 때
- CONSTITUTION 불변 원칙과 현실이 충돌할 때
- 작업 범위가 예상의 3배 이상 커질 것 같을 때
- 외부 의존성(패키지, API, DB 스키마) 변경이 필요할 때
- 같은 오류로 3회 이상 재시도 실패했을 때
- `open-decisions`의 🔴 Blocker 항목을 만났을 때
- 작업 도중 "작은 기능"이 "큰 기능"으로 커진 것을 감지했을 때

---

## 7. 세션 종료 루틴 (Level 1)

에이전트는 작업 종료 시 반드시 수행:

1. `PLANS.md` 체크리스트 업데이트
2. `open-decisions.md` 새 항목 추가 (있으면)
3. `tech-debt-tracker.md` 업데이트 (있으면)
4. **다음 세션 핸드오프 작성** → `docs/exec-plans/NEXT_SESSION.md`

### 핸드오프 메시지 형식 (Core 표준)

```markdown
## 🔖 다음 세션 핸드오프

**현재 Phase**: [Phase N]
**마지막 완료**: [기능명]
**다음 할 것**: [작업명 + 간략 설명]

**막힌 것**: (없으면 생략)
- [항목]: [이유]

**결정 필요**: (없으면 생략)
- [항목]: [선택지 A / B]

**세션 종료 시각**: YYYY-MM-DD HH:MM
```

---

## 8. Phase 루프 (Level 3)

### 진입 조건
- 이전 Phase 체크리스트 모두 완료
- `phase-gates.md` 업데이트 (사용하는 경우)
- 사람의 명시적 승인

### 완료 선언 시 에이전트가 확인할 것
- `PLANS.md`의 해당 Phase 전체 체크 완료
- `open-decisions`의 해당 Phase 관련 항목 모두 처리됨
- `tech-debt`의 Phase 차단 항목 없음
- 사람에게 다음 Phase 진입 확인 요청

---

## 9. 비정상 종료 복구

세션이 비정상 종료된 경우(네트워크, 컨텍스트 오버플로 등), 다음 세션 시작 시:

1. `NEXT_SESSION.md` 확인
2. 없으면 `git log`로 마지막 커밋 확인
3. `PLANS.md`와 실제 구현 상태 대조
4. 사람에게 복구 상태 보고 후 재개
