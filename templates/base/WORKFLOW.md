...
4. `PLANS.md` 현재 위치 + `docs/exec/open-decisions.md` 미결 항목 확인
5. 사람에게 한 줄 확인:
...
기능 루프 시작 전, 에이전트는 작업 크기를 먼저 판단한다.

### 작은 기능 (Small)
아래 **3가지 모두** 해당:
- 데이터 스키마 변경 없음 (DB, 저장 포맷, API 스펙, JSON 스키마 등)
- 외부 API 신규 연동 없음
...
> ⚠️ 이 기준은 운영하면서 조정 가능. 너무 자동화되면 엄격하게, 너무 번거로우면 느슨하게.
...
1. `PLANS.md` 체크리스트 업데이트
2. `docs/exec/open-decisions.md` 새 항목 추가 (있으면)
3. `docs/exec/tech-debt.md` 업데이트 (있으면)
4. **다음 세션 핸드오프 작성** → `docs/exec/NEXT_SESSION.md`
...
### 완료 선언 시 에이전트가 확인할 것
- `PLANS.md`의 해당 Phase 전체 체크 완료
- `docs/exec/open-decisions.md`의 해당 Phase 관련 항목 모두 처리됨
- `docs/exec/tech-debt.md`의 Phase 차단 항목 없음
