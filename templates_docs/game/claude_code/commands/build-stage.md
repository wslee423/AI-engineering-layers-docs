...
→ Main Session이 Large로 분류
→ Implementer 호출 (스테이지 데이터 생성 + 자체 검증)
→ Reviewer 호출 (QUALITY_SCORE.md §6 스테이지 검증 기준 적용)
```

자세한 흐름은 `templates_docs/game/claude_code/commands/orchestrate.md` 참조.
...
분리 시 추가될 파일:
- `templates_docs/game/claude_code/agents/stage-builder.md`
